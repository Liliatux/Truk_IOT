...
class Select extends Component {
  ...
  render() {
    let items = this.state.items.map(item => {
      ...
      return <option  id="option" onChange={this.update.bind(this)} className="item" value={tab} key={item.Entreprise}><img src={item.Logo} alt="logo" className="img-select"/>{item.Entreprise}</option>
    });
